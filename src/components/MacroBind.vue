<template>
  <div
    class="outline-none h-10 m-auto px-3 flex items-center justify-between rounded border border-[#DCDFE6] dark:border-[#4c4d4f] hover:border-[#c0c4cc] dark:hover:border-[#6c6e72] focus:border-[#409EFF] bg-white dark:bg-slate-800 select-none hover:cursor-pointer"
    @click="onClick"
    @blur="onBlur"
...
function onKeydown(e) {
  if (e.repeat) return;
  if (keySet.value.split("+").length > 2) return;
